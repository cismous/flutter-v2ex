...
class RepliesResp extends BaseResp {
  ...
  RepliesResp.fromParams(
      ...
      : super.fromParams(
            message: message, status: status, rateLimit: rateLimit);
...
class Reply {
  ...
  @override
  String toString() {
    return '{"created": $created,"id": $id,"lastModified": $lastModified,"lastTouched": $lastTouched,"replies": $replies,"content": ${content != null ? '${json.encode(content)}' : 'null'},"contentRendered": ${contentRendered != null ? '${json.encode(contentRendered)}' : 'null'},"lastReplyBy": ${lastReplyBy != null ? '${json.encode(lastReplyBy)}' : 'null'},"title": ${title != null ? '${json.encode(title)}' : 'null'},"url": ${url != null ? '${json.encode(url)}' : 'null'},"member": $member}';
  }
}
